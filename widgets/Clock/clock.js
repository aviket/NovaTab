...
export class ClockWidget extends BaseWidget {
    ...
    startTicker() {
        const update = () => {
            ...
            if (elS) elS.style.transform = `translate(-50%, -100%) rotate(${s * 6}deg)`;
            if (elM) elM.style.transform = `translate(-50%, -100%) rotate(${m * 6 + s * 0.1}deg)`;
            if (elH) elH.style.transform = `translate(-50%, -100%) rotate(${(h % 12) * 30 + m * 0.5}deg)`;

            const dTime = this.element.querySelector('#digitalTime');
            const dDate = this.element.querySelector('#digitalDate');

            if (dTime) dTime.textContent = now.toLocaleTimeString([], { hour12: false });
            if (dDate) dDate.textContent = now.toLocaleDateString(undefined, {
                weekday: 'short',
                year: 'numeric',
                month: 'short',
                day: 'numeric'

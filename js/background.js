...
function looksLikeNewTab(tab = {}, changeInfo = {}) {
  ...
  const changeTitle = typeof changeInfo.title === 'string' ? changeInfo.title : '';
  ...
  if (tabUrl.startsWith('chrome://newtab') || changeUrl.startsWith('chrome://newtab')) {
    if (tabTitle.includes(EXPECTED_TITLE_SUBSTR) || changeTitle.includes(EXPECTED_TITLE_SUBSTR)) {
      return true;
    }
    return false;
  }
  ...
  if ((tabTitle && tabTitle.includes(EXPECTED_TITLE_SUBSTR)) && (changeInfo.url || changeInfo.title)) {
    return true;
  }
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  try {
    ...
    getTabsByTitle(/NovaTab/i).then(newtabs => {
      ...
      closeDuplicateTabs(newtabs).then(res => {
...
function getTabById(tabId) {
  return new Promise((resolve, reject) => {
    ...
    chrome.tabs.get(tabId, (tab) => {
      if (chrome.runtime.lastError) {
        return reject(new Error('chrome.tabs.get error: ' + chrome.runtime.lastError.message));
      }
      resolve(tab);
    });
  });
...
function getTabsByTitle(pattern, queryOpts = {}) {
  const rx = (pattern instanceof RegExp) ? pattern : new RegExp(String(pattern), 'i'); // case-insensitive by default
  return new Promise((resolve, reject) => {
    chrome.tabs.query(queryOpts, (tabs) => {
      if (chrome.runtime.lastError) {
        return reject(new Error('chrome.tabs.query error: ' + chrome.runtime.lastError.message));
      }
      const matches = (tabs || []).filter(t => {
...
function setTabPinned(tabId, pinned = true) {
  return new Promise((resolve, reject) => {
    ...
    chrome.tabs.update(tabId, { pinned }, (tab) => {
      if (chrome.runtime.lastError) {
        return reject(new Error('chrome.tabs.update error: ' + chrome.runtime.lastError.message));
      }
      resolve(tab);
    });
  });
...
async function closeDuplicateTabs(tabsArr = []) {
  ...
  const extractId = (item) => {
    ...
    if (typeof item === 'object' && item.id !== undefined) return Number(item.id);
    return null;
  };
  ...
  const tabPromises = (tabsArr || []).map(item => {
    ...
    if (item && typeof item === 'object' && item.id !== undefined && item.windowId !== undefined) {
    ...
    return new Promise((resolve) => {
      chrome.tabs.get(id, (tab) => {
        if (chrome.runtime.lastError) {
          console.warn('closeDuplicateTabs: chrome.tabs.get failed for id', id, chrome.runtime.lastError.message);
          return resolve(null);
        }
        resolve(tab);
      });
  ...
  if (tabsResolved.length === 1) {
    ...
    try {
      ...
      return { keeper: keeperTab, removed: [], pinned: keeperTab.pinned, activated: !!keeperTab.active };
    }
  }
    ...
    if (!pinned) {
      await new Promise((resolve) => {
        chrome.tabs.update(keeper.id, { pinned: true }, (updated) => {
          if (chrome.runtime.lastError) {
            console.warn('closeDuplicateTabs: failed to pin keeper', chrome.runtime.lastError.message);
            return resolve(null);
          }
  ...
  try {
    await new Promise((resolve) => {
      chrome.windows.update(keeper.windowId, { focused: true }, (win) => {
        if (chrome.runtime.lastError) {
          console.warn('closeDuplicateTabs: windows.update failed', chrome.runtime.lastError.message);
          return resolve(null);
        }
        windowFocused = true;
  ...
  try {
    await new Promise((resolve) => {
      chrome.tabs.update(keeper.id, { active: true }, (tab) => {
        if (chrome.runtime.lastError) {
          console.warn('closeDuplicateTabs: tabs.update(active) failed', chrome.runtime.lastError.message);
          return resolve(null);
        }
    ...
    if (duplicatesIds.length) {
      await new Promise((resolve) => {
        chrome.tabs.remove(duplicatesIds, () => {
          if (chrome.runtime.lastError) {
            console.warn('closeDuplicateTabs: tabs.remove error', chrome.runtime.lastError.message);
